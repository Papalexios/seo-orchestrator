...
export const AiConfiguration: React.FC<AiConfigurationProps> = ({ onConfigured, currentConfig }) => {
  ...
  const [strategy, setStrategy] = useState<'parallel' | 'fallback' | 'single'>(currentConfig?.strategy || 'parallel');
  const [stageOverrides, setStageOverrides] = useState(currentConfig?.stageOverrides || {});
  ...
  useEffect(() => {
    const handler = setTimeout(() => {
        ...
        if (provider === 'openrouter') {
            ...
            configToValidate.strategy = strategy;
            configToValidate.stageOverrides = stageOverrides;
        } else {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (validationState === 'valid') {
      ...
      if (provider === 'openrouter') {
        ...
        configToSave.strategy = strategy;
        configToSave.stageOverrides = stageOverrides;
      } else {
    ...
    <div className="bg-gray-900/50 p-6 sm:p-8 rounded-xl shadow-2xl border border-gray-700/60 animate-fade-in">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div>
              ...
              <div className="flex gap-2">
                <input
                    ...
                    placeholder="e.g. deepseek/deepseek-chat, zhipu/glm-4.5-flash"
        ...
        {provider === 'openrouter' && (
          <div className="animate-fade-in space-y-4">
            <div>
              <label className="block text-sm font-semibold text-gray-400 mb-1">Concurrency Strategy</label>
              <div className="grid grid-cols-3 gap-2">
                {(['parallel','fallback','single'] as const).map(opt => (
                  <button key={opt} type="button" onClick={() => setStrategy(opt)} className={`p-2 rounded-lg border-2 ${strategy === opt ? 'bg-blue-600/20 border-blue-500' : 'bg-gray-800/60 border-gray-700 hover:border-gray-600'}`}>
                    <span className="text-sm capitalize text-gray-200">{opt}</span>
                  </button>
                ))}
              </div>
              <p className="text-xs text-gray-500 mt-1.5">Parallel: fastest first response. Fallback: try models sequentially for reliability. Single: always use first model.</p>
            </div>

            <div>
              <label className="block text-sm font-semibold text-gray-400 mb-1">Per‑Stage Model Overrides (optional)</label>
              <div className="grid sm:grid-cols-2 gap-3">
                {[
                  { key: 'sitewide', label: 'Sitewide Audit' },
                  { key: 'pageLevel', label: 'Page Analysis' },
                  { key: 'actionPlan', label: 'Action Plan' },
                  { key: 'executive', label: 'Executive Summary' },
                  { key: 'selfCorrection', label: 'Self‑Correction' },
                  { key: 'serp', label: 'SERP Deconstruction' },
                ].map(s => (
                  <div key={s.key}>
                    <label className="text-xs text-gray-400">{s.label}</label>
                    <input
                      type="text"
                      value={(stageOverrides?.[s.key as keyof typeof stageOverrides]?.model) || ''}
                      onChange={(e) => setStageOverrides(prev => ({ ...prev, [s.key]: { model: e.target.value } }))}
                      placeholder="optional model name (e.g., openai/gpt-4o)"
                      className="w-full px-3 py-2 bg-gray-800/80 text-gray-200 border border-gray-700 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none text-sm"
                    />
                  </div>
                ))}
              </div>
              <p className="text-xs text-gray-500 mt-1.5">Overrides take precedence over the global model list for each stage.</p>
            </div>
          </div>
        )}
