...
export const SerpInsightsDisplay: React.FC<SerpInsightsDisplayProps> = ({ insights, aiProvider }) => {
    ...
    return (
        <div className="mt-4 bg-gray-900/50 p-6 rounded-xl border border-indigo-700/60 relative animate-fade-in shadow-2xl space-y-8">
            ...
            <section>
                <div className="flex items-center justify-between gap-4 mb-3"> <div className="flex items-center gap-3"> <div className="p-2 bg-green-900/50 rounded-lg border border-green-700/60"><RelatedSearchesIcon /></div> <h4 className="font-semibold text-lg text-gray-200">Related Searches</h4> </div> {relatedSearches?.length > 0 && <ListCopyButton textToCopy={relatedSearches.join('\n')} />} </div>
                <div className="p-4 bg-gray-950/70 rounded-lg border border-gray-700/80"> {relatedSearches?.length > 0 ? <div className="flex flex-wrap gap-3"> {relatedSearches.map((query, index) => ( <span key={index} className="text-base text-gray-300 bg-gray-800/80 px-3 py-1.5 rounded-full font-medium"> {query} </span> ))} </div> : <p className="text-sm text-gray-500 italic">No "Related Searches" were found.</p>} </div>
            </section>
        </div>
    );
};
