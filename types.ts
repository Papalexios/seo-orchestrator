...
// Feedback log entries for self-correction
export interface FeedbackLogEntry {
    timestamp: string; // ISO date
    level: 'info' | 'warning' | 'critical';
    message: string; // concise thought
}

export interface SelfCorrectionFeedback {
    critiqueSummary: string; // high-level critique
    missedOpportunities: string[]; // bullets
    refinements: string[]; // recommended refinements
    riskWarnings: string[]; // potential risks
    modelTrace: FeedbackLogEntry[]; // agent thoughts log
}
...
export interface HistoricalAnalysis {
  ...
  selfCorrection?: SelfCorrectionFeedback; // new
}
...
export interface AiConfig {
  ...
  strategy?: 'parallel' | 'fallback' | 'single'; // OpenRouter strategy
  stageOverrides?: {
    sitewide?: { model?: string };
    pageLevel?: { model?: string };
    actionPlan?: { model?: string };
    executive?: { model?: string };
    selfCorrection?: { model?: string };
    serp?: { model?: string };
  };
}
...
export interface GenerateContentParameters {
    model: string;
    contents: any[] | string; // Loosely typed to avoid strict dependency
    config: {
