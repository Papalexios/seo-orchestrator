...
export const generateReportMarkdown = (analysis: HistoricalAnalysis): string => {
    ...
    if (analysis.selfCorrection) {
        const fb = analysis.selfCorrection;
        report += `## 🧪 Agent Feedback Log (Self-Correction)\n\n`;
        report += `${fb.critiqueSummary}\n\n`;
        if (fb.missedOpportunities.length) {
            report += `### Missed Opportunities\n` + fb.missedOpportunities.map(m => `- ${m}`).join('\n') + `\n\n`;
        }
        if (fb.refinements.length) {
            report += `### Refinements\n` + fb.refinements.map(r => `- ${r}`).join('\n') + `\n\n`;
        }
        if (fb.riskWarnings.length) {
            report += `### Risk Warnings\n` + fb.riskWarnings.map(r => `- ${r}`).join('\n') + `\n\n`;
        }
        if (fb.modelTrace.length) {
            report += `### Agent Thoughts (Trace)\n` + fb.modelTrace.map(t => `- [${new Date(t.timestamp).toUTCString()}] (${t.level}) ${t.message}`).join('\n') + `\n\n`;
        }
        report += `------------------------------\n\n`;
    }
