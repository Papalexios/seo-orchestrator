...
const generateExecutiveSummaryMarkdown = (summary: ExecutiveSummary): string => {
    ...
    if (summary.rewrites && summary.rewrites.length > 0) {
        markdown += `### ✍️ Top Pages to Rewrite\n`;
    ...
    if (summary.newContent && summary.newContent.length > 0) {
    ...
    if (summary.redirects && summary.redirects.length > 0) {
        markdown += `### ↪️ Critical Redirects\n`;
