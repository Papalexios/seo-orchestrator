...
import { executeConcurrent, getAdaptiveConcurrency, getAdaptiveChunkSize } from './utils/utility';
...
const App: React.FC = () => {
  ...
  const handleSubmit = useCallback(async (data: WizardSubmitData) => {
    ...
    try {
      ...
      const CHUNK_SIZE = getAdaptiveChunkSize(rankedUrls.length);
      const CONCURRENCY_LIMIT_ANALYSIS = getAdaptiveConcurrency(aiConfig.provider, aiConfig.strategy);
      const combinedAnalysis: SeoAnalysisResult = { pageActions: [], keywords: [] };

      // If GSC is connected, batch fetch performance data for URLs in this chunk
      const gscEnabled = !!gscToken && !!data.url;
      const getGscMapForChunk = async (chunk: string[]) => {
        if (!gscEnabled) return undefined;
        const siteUrl = data.url;
        const entries = await Promise.all(chunk.map(async (u) => {
        // cache GSC performance per URL for 12 hours to reduce repeated requests
        const key = (u: string) => `gsc:${u}`;
        const readCache = (u: string) => {
          try { const raw = localStorage.getItem(key(u)); if (!raw) return null; const { v, t } = JSON.parse(raw); if (Date.now()-t < 1000*60*60*12) return v; } catch{}
          return null;
        };
        const writeCache = (u: string, v: any) => { try { localStorage.setItem(key(u), JSON.stringify({ v, t: Date.now() })); } catch{} };

          const cached = readCache(u);
          if (cached) return [u, cached] as const;
          try {
            const perf = await fetchGscPerformanceForUrl(u, siteUrl, gscToken!.access_token);
            if (perf) writeCache(u, perf);
            return [u, perf] as const;
          } catch {
      ...
      const analysisResults = await executeConcurrent(
        urlChunks,
        async (chunk) => {
            const gscMap = await getGscMapForChunk(chunk);
            return generateSeoAnalysis(
              aiConfig,
              chunk,
              data.analysisType,
              data.targetLocation,
      ...
      log('Generating Full Day-by-Day Action Plan...', 'running');
      const actionPlan = await generateActionPlan(aiConfig, sitewideAnalysis, analysis, (msg) => log(msg));
      ...
      // Self-correction pass (cached by analysis id)
      log('Initiating Autonomous Self-Correction...', 'running');
      const scCacheKey = `selfCorrection:${new Date().toISOString()}`; // simple unique id per run
      let selfCorrection = null;
      try {
        const run = async () => await generateSelfCorrection(aiConfig, sitewideAnalysis, analysis, actionPlan);
        const { memoizeLocal } = await import('./utils/utility');
        selfCorrection = await memoizeLocal(scCacheKey, 1000 * 60 * 60, run)();
        log('Self-Correction pass complete.', 'complete');
      } catch (e) {
        console.warn('Self-correction failed:', e);
        log('Self-Correction encountered an issue, continuing without it.', 'error');
      }
      ...
      const newAnalysis: HistoricalAnalysis = {
        ...
        selfCorrection: selfCorrection || undefined,
      };
